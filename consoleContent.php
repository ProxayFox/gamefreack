...
$resultGameQuery = DB::queryFirstRow("
  SELECT *
  FROM consoleInventory 
  WHERE CIID = '".$_GET['CIID']."'
");

$resultUserReview = DB::query("
  SELECT userconsolecomment.date, userConsoleComment.comment, clientprofile.displayName, clientprofile.UIMG
  FROM userConsoleComment RIGHT JOIN clientprofile
    ON userConsoleComment.CPID = clientprofile.CPID
  ORDER BY RAND() LIMIT 6
");

$resultGuestReview = DB::query("
  SELECT username, date, comment
  FROM guestConsoleComments 
  WHERE CIID = '".$_GET['CIID']."'
");
